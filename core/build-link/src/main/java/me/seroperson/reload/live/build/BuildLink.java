...
public interface BuildLink {
	 ...
	 *
	 * <p>
	 * When the application throws an exception, this will be called for every
	 * element in the stack trace from top to bottom until a source file may be
	 * found, so that the browser can render the line of code that threw the
	 * exception.
	 *
	 * <p>
	 * If the class is generated (eg a template), then the original source file
	 * should be returned, and the line number should be mapped back to the line
	 * number in the original source file, if possible.
	 *
	 * @param className The name of the class to find the source for.
	 * @param line      The line number the exception was thrown at.
	 * @return Either:
	 *         <ul>
	 *         <li>[File, Integer] - The source file, and the passed in line number,
	 *         if the source wasn't generated, or if it was generated, and the line
	 *         number could be mapped, then the original source file and the mapped
	 *         line number.
	 *         <li>[File, null] - If the source was generated but the line number
	 *         couldn't be mapped, then just the original source file and null for
	 *         the unmappable line number.
	 ...
	 *
	 * <p>
	 * This is invoked by plugins for example that change something on the classpath
	 * or something about the application that requires a reload, for example, the
	 * evolutions plugin.
	 */
	void forceReload();
