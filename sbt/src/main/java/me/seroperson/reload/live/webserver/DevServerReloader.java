...
class DevServerReloader implements BuildLink, Closeable {
	...
	private Object reload(boolean shouldReload) {
		...
		if (compileResult instanceof CompileFailure) {
			...
			long newLastModified = cp.stream().filter(File::exists).flatMap(DevServerReloader::listRecursively)
	...
	/**
	 * Contrary to its name, this doesn't necessarily reload the app. It is invoked
	 * on every request, and will only trigger a reload of the app if something has
	 * changed.
	 *
	 * <p>
	 * Since this communicates across classloaders, it must return only simple
	 * objects.
	 *
	 * @return Either<br>
	 *         - {@link Throwable} - If something went wrong (eg, a compile error).
	 *         <br>
	 *         - {@link ClassLoader} - If the classloader has changed, and the
	 *         application should be reloaded.<br>
	...
	private static Stream<File> listRecursively(File file) {
		try {
			return Files.walk(file.toPath()).filter(path -> !(Files.isDirectory(path) && path.equals(file.toPath())))
					.map(Path::toFile);
		} catch (IOException e) {
